...
def plot_total_intensities(mie_scatt_wavelength: dict) -> None:
    ...
    ax1.set_xlabel(r"Scattering Angle (rad)")
    ax1.set_ylabel(r"Parallel Intensity")
    ax2.set_xlabel(r"Scattering Angle (rad)")
    ax2.set_ylabel(r"Perpendicular Intensity")
    ax3.set_xlabel(r"Scattering Angle (rad)")
...
def plot_intensity_polarization(theta, SL, SR, SU, P, title=None) -> None:
    """
    ...
    if title is None:
        title = 'Combined Scattering Function and Polarization Of All Wavelengths and Particle Sizes'
    plt.title(title)
    plt.show(block=False)


def plot_mueller_matrix_elems(theta, S11, S12, S33, S34, title=None) -> None:
    """
    ...
    ax4.set_xlabel(r"Scattering Angle (rad)")
