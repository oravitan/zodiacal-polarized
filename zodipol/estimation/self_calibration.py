...
class SelfCalibration:
    ...
    def forward_model(self, obs=None, p=None, eta=None, delta=None, alpha=None):
        ...
        for ii in range(len(obs)):
            ...
            I, Q, U = obs[ii].I, obs[ii].Q, obs[ii].U
    ...
    def _calibrate_property(self, property_name):
        rng = np.linspace(*self.get_property_bounds(property_name), 20)
        img = np.stack([self.forward_model(**{property_name: x*np.ones_like(getattr(self, property_name))}) for x in rng], axis=-1)
        diff_resh = img - self.images.value[..., None]
        cost = np.log10((diff_resh ** 2).sum(axis=1))
        mincost = np.argmin(cost, axis=-1)
        prop_val = rng[mincost]
        return prop_val
