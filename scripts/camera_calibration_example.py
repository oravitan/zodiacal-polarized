...
def get_initial_parameters(obs, parser, zodipol):
    ...
    obs_orig = [zodipol.make_camera_images(o, polarizance_real, polarization_angle_real, n_realizations=1, add_noise=True) for o in obs_biref]
...
if __name__ == '__main__':
    ...
    p, eta, delta, alpha, cost, itr_cost = calib.calibrate(images_orig, n_itr=5, mode="all", callback=callback_partial)
    ...
    ax[0, 0].imshow(polarizance_real.reshape((parser["resolution"][0], parser["resolution"][1])), cmap='gray', vmin=0.8, vmax=0.9)
    ax[0, 0].set_title("$P^{true}$")
    ax[0, 1].imshow(p.reshape((parser["resolution"][0], parser["resolution"][1])), cmap='gray', vmin=0.8, vmax=0.9)
    ax[0, 1].set_title("$\hat{P}$")
    ax[1, 0].imshow(polarization_angle_real.reshape((parser["resolution"][0], parser["resolution"][1])), cmap='gray', vmin=-np.pi / 12, vmax=np.pi / 12)
    ax[1, 0].set_title("$\eta^{true}$")
    ax[1, 1].imshow(eta.reshape((parser["resolution"][0], parser["resolution"][1])), cmap='gray', vmin=-np.pi / 12, vmax=np.pi / 12)
