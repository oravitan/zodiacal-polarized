...
def get_mie_scattering():
    ...
    """
    theta, S11, S12, S33, S34 = np.loadtxt('mie_scattering/outputs/mie_scattering.csv', delimiter=',', skiprows=1, unpack=True)
...
def get_mueller_matrix_from_elem(S11: np.ndarray, S12: np.ndarray, S33: np.ndarray, S34: np.ndarray):
    ...
    M = np.zeros((4, 4, len(S11)), dtype=np.complex128)
    M[0, 0, :] = S11
    M[0, 1, :] = S12
    M[1, 0, :] = S12
    M[1, 1, :] = S11
...
def get_mueller_matrix(theta: np.ndarray) -> np.ndarray:
    """
